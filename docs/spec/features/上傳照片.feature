...
Feature: 上傳照片
  ...
  Rule: 照片受理後分析成功，系統儲存照片資訊（VLM 理解照片內容後轉成的文字）
  ...
  Rule: 照片受理後分析成功，系統儲存 VLM 產生的結構化 metadata（照片類別、地點/商家、物品清單、內容時間；清單外資訊捨棄），其中照片類別在入庫當下一律為「未分類」
  ...
  Rule: 照片受理後分析成功，系統儲存透過 LangChain 產生的 embedding 向量（由文字與 metadata 合併之內容產生）
    Example: 上傳照片後產生 embedding 向量
      When 使用者上傳一張照片，VLM 理解其內容為 "在 Target 購買可樂與洋芋片的收據，日期 2026-08-10"
      Then 照片的 embedding 不為空

  Rule: 照片受理後分析成功，系統記錄上傳時間
    Example: 上傳照片後記錄上傳時間
      Given 現在時間為 "2026-08-18 10:00"
      When 使用者上傳一張照片，VLM 理解其內容為 "在 Target 購買可樂與洋芋片的收據，日期 2026-08-10"
      Then 照片的上傳時間為 "2026-08-18 10:00"

  Rule: 照片受理後分析成功，系統保留原始照片檔與縮圖
    Example: 上傳照片後保留原圖與縮圖
      When 使用者上傳一張照片，VLM 理解其內容為 "在 Target 購買可樂與洋芋片的收據，日期 2026-08-10"
      Then 照片的原圖與縮圖都已儲存
      And 回應包含這張照片的縮圖網址

  Rule: 照片分析成功後，系統提供照片識別碼、文字描述與 metadata
    Example: 上傳成功的回應內容
      When 使用者上傳一張照片，VLM 理解其內容為 "在 Target 購買可樂與洋芋片的收據，日期 2026-08-10"
      Then 回應包含照片識別碼
  ...
  Rule: 照片分析成功後，系統提供照片所屬資料夾、VLM 建議的資料夾與完整資料夾清單
  ...
  Rule: 照片分析成功後，系統提供 VLM 建議的實體與完整實體清單，且尚未釘上任何實體
  ...
  Rule: VLM 建議的實體不在實體清單中時，不當成新實體、也不留下建議
  ...
  Rule: 照片分析成功後，系統提供 VLM 建議的待辦，且尚未建立待辦

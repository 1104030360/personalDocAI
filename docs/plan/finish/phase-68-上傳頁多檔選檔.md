...
> ⚠ **（2026-08-26 校準：上面那段「現在的上傳頁」講的是「增量五之前」，不是你打開瀏覽器
> 會看到的現況。）** Phase 62 已經進 commit `f1a7e71`，後端現在回 202，而 `upload.html`
> **還停在 201 的寫法**（`if (response.status === 201)`，第 179 行）——所以此刻上傳一張圖，
> 頁面會走 `else` 分支印出紅字「失敗（HTTP 202）」，底下再印一行 `undefined`
> （202 的回應沒有 `detail` 這個鍵）。檔案其實**有**被收下、job 也真的建了。
> 這是階段乙做完、階段丙還沒接上的**預期過渡狀態**（總覽 §1.3 講的就是這件事），
> 不是壞掉，也不必先去「修一下」——本 phase §4.6 整段換掉之後它自己就對了。
...
| **65／66** | （2026-08-26 校準：原表漏列）Celery worker ＋ Redis JobStore ＋ Compose 的 `worker` 服務。**自動化契約測試不需要它們**（那三顆只掃原始碼字串），但 §6.2 的第 4、6、11 項（成功列自己消失、頂欄 N 加上去、PDF 拆頁頁碼、照片真的出現在待決定）全部要靠 worker 真的把 job 做完——階段乙做完的當下 `get_task_dispatcher()` 回的是 `NoopDispatcher`，檔案只會停在 `queued`。 |
...
1. `app/static/upload.html`：
   ...
   - `<script src>` 那一區：拿掉四個彈窗相關的檔
     （2026-08-26 校準：`progress_panel.js` 那一行是 **Phase 67 §4.4 已經加好的**，
     本 phase 只是把它留著，不要再加一次）
...
>
> （2026-08-26 校準：上面那個「+2」是 **Phase 67 做完之後**才成立的淨值，
> 中間會先胖一段。Phase 53 其實加了 **26 行**——頂欄那一格 1 行 ＋ `</header>`
> 正下方 25 行的過渡計數片段；Phase 67 §4.5 再把那 25 行整組刪掉、另加 1 行
> `<script src>`。所以在**還沒做 67 的 HEAD** 上開 `upload.html` 對行號，
> 看到的是 `<p class="lead">` 在第 65 行（＝ +26）、內嵌 `<script>` 在第 89 行，
> **那不是計畫寫錯**。67 做完之後才會落在「lead ≈ 40、內嵌 `<script>` ≈ 65」。）

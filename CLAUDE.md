...
**2026-09-03 改判：EC2 改 GPU 機自裝 Ollama（design6 D12 作廢；總覽 §10.2 追認項 T；產品負責人拍板「留著開關」）**：工人看圖後端由 `config.WORKER_VLM_BACKEND` 決定——`cloud`（預設；留空或不填也算）＝ollama.com、`local`＝**工人所在那台機器**的 Ollama（GPU EC2 用這個；`OLLAMA_BASE_URL`／`VLM_MODEL`，不需要 `OLLAMA_API_KEY`）；`cloud_worker.build_worker_vlm()` 分流、打錯字當場炸、啟動行尾多 `vlm=… model=…`；`process_job_message` 六規則與 S3／SQS 契約一字未動。`deploy/ec2/`：Ollama 跑 host（官方 `ollama.service`、只聽 127.0.0.1），工人容器 `docker run --network host`，unit 加 `After=/Wants=ollama.service`＋等 `/api/tags` 120 秒的 `ExecStartPre`（`{1..60}`）；user-data 先裝 unit＋enable 再裝 Ollama＋`ollama pull gemma4:e2b`（非致命）；`worker.env.example` 多 `WORKER_VLM_BACKEND`／`OLLAMA_BASE_URL`／`VLM_MODEL`（值不加引號）。Mac 上 `WORKER_VLM_BACKEND=local` 完整回合實測通過（本機看圖 90 秒、`雲端結果已入庫 #73`）。ECR `latest` 改推**多架構**（amd64＋arm64）manifest、tag `bb3921a-dirty`（未 commit）。**2026-09-03 再改判：phase-92 拆兩段（總覽 §10.2 追認項 U）**——**92-A（現在做）** `t3.xlarge`（x86_64、一般 AL2023、**30 GB gp3**、`worker.env` 填 `WORKER_VLM_BACKEND=cloud`），把整條 AWS 流程與 Demo 2／2b 驗完，**收工 Stop**（30 GB ≈ $2.9／月，在 $5 Budget 內，留給 Phase 94 的 Demo 3）；前置只要 Standard 配額 `L-1216C47A`（本帳號實查 **8 vCPU**，新帳號 Standard 預設常是 5，**不必申請**）。**92-B（配額核准後）** 先 Terminate 92-A → `g4dn.xlarge`＋Deep Learning Base OSS NVIDIA GPU AMI＋**80 GB gp3**＋`local` → 多驗 `nvidia-smi`／`ollama ps` 兩關 → 測完 **Terminate**（80 GB 停著 ≈ $7.7／月，超過 Budget）。**★G3 移到 92-A 之後**（93／94／95 都不依賴 GPU）。**G and VT 配額 `L-DB2E81BA` 目前 0、申請已送出、狀態 `CASE_OPENED`——不要重送**。兩台**都是 x86_64**，多架構映像那條不變；兩段共用同一份 `deploy/ec2/` 三檔。⚠ 92-A 若要在同一台試 `local` 硬跑 CPU 推論，**必須先把 Mac `.env` 的 `CLOUD_RESULT_TIMEOUT_SECONDS` 從 300 暫調 900 並 restart worker，做完改回 300**（否則本機先 `fallback=local reason=result_timeout`、工人稍後放好的 `result.json` 變孤兒）。phase-92-A 已實作並驗收；**93／94／95 已於 2026-09-03 完成（見「增量六收官」段）**。要改回純雲端推論：`worker.env` 留空 `WORKER_VLM_BACKEND` 即可，零程式碼。啟動時四個看圖設定（cloud：`OLLAMA_API_KEY`＋`OLLAMA_CLOUD_VLM_MODEL`；local：`OLLAMA_BASE_URL`＋`VLM_MODEL`）都會檢查，缺了大聲退出。測試 **690**（2026-09-03 晚 review 補一顆 `test_AWS_REGION留空等於東京`）→ owner `f2fc067` 補 2 顆＝**692**；**本輪（Phase 93〜95）收工 716**。

**增量六收官（Phase 93〜95，2026-09-03；dev-prompt `phase0903-1.md`；未 commit）**：**93** GitHub OIDC 鑰匙——AWS 上建 OIDC provider（`token.actions.githubusercontent.com`，不填 thumbprint、AWS 自取）＋ IAM role `personaldocai-github-deploy`（trust 的 `sub` **逐字** `repo:1104030360@92135456/personalDocAI@1349196211:ref:refs/heads/main`＝GitHub 不可變主體格式，`StringEquals`、整份零 `*`；inline policy `personaldocai-github-deploy-policy` 恰五段：ECR 登入票／推 `personaldocai-worker` 六動作／`ssm:SendCommand` 綁那一台實例＋`arn:aws:ssm:ap-northeast-1::document/AWS-RunShellScript`（兩個冒號是對的）／`GetCommandInvocation`／`DescribeInstances`；max session 3600、零 managed policy）；repo 裡 `deploy/aws/github-oidc-trust.json`／`github-deploy-policy.json` 只有 `<ACCOUNT_ID>`／`<INSTANCE_ID>` 佔位（送 AWS 時才 `sed` 展開到專案外）；GitHub secret `AWS_DEPLOY_ROLE_ARN`；4 顆掃碼（sub 鎖 main／零萬用字元／aud／`deploy/aws/*.json` 零 12 位數字）。**94** CD——`.github/workflows/deploy.yml`（`workflow_run` 綁 `test`、`branches: [main]`、job `if` 恰 `event == 'push' && conclusion == 'success'`（擋 fork PR）、`permissions` 只 `id-token: write`＋`contents: read`、checkout **`head_sha`**（CI 測過的那版，不是當下 main）、QEMU＋buildx `linux/amd64,linux/arm64`、`target: cloud-worker`、`GIT_SHA=head_sha`、tags `<sha>`＋`latest`、`cache type=gha`；最後一步 EC2 **running 才** SSM `systemctl restart personaldocai-worker`（unit 的 `ExecStartPre` 會 pull latest），stopped／terminated／找不到（`describe-instances` 失敗吞成 `not-found`）／變數沒設 → `::notice::`＋`exit 0`；SSM 輪詢 60×10s＝10 分鐘（蓋過 unit `TimeoutStartSec=600`）；不填 `role-duration-seconds`）；GitHub variable `EC2_WORKER_INSTANCE_ID`；6 顆掃碼（**regex 一律行首錨定＋`re.M`**——review 抓到未錨定的 `branches: [main]` 被 deploy.yml 自己的註解餵綠）；README §9 英文 `### CI/CD`。⚠ **Demo 3 尚未跑**：`workflow_run` 無法本機模擬，要產品負責人 commit → push（origin/main 落後 10 個 commit、CI 自 08-28 沒跑，第一次 push 可能紅在別處，先修 CI、不動 `test.yml`）→ 改一行註解再 push → `test` 綠 → `deploy` 綠 → ECR 有 `<sha>` → Start 92-A → `docker logs cloud-worker | head -1` 的 `version=<sha>` → Stop。**95** 收尾——design6 §8 錯誤表 10 列逐列點名（20 顆既有測試全點得到；`敏感中文關鍵字`／`空檔名`／`亂碼檔名` 三個舊名隨 09-01 閘門改判作廢、由 `test_檔名完全不影響判斷` 守）＋兩個真缺口（雲端三次看不懂＝整筆失敗**不**再走本機；遠端不可用仍 202 三鍵）＋八顆掃碼（NAT／EIP／ALB／Lambda／ECS 字樣（**詞邊界**：裸 `nat`／`ecs` 會誤中 `Terminate`／`ExecStartPre`）、compose 零 AWS 變數名與工人服務名、端點 22 零 DELETE、佇列 body 無位元組（真 `AwsMailbox` 序列化）、工人不寫 DB 不算 embedding（ast：識別字／字串常數／動態載入，與 87 的 import 掃碼互補）、boto3 只在 `aws_mailbox.py`＋其單元測試＋`aws_check.py`、photo 表 16 欄整集合相等、閘門不寫 `AI_BACKEND`）＋前輪停放項（`cloud_worker.read_context` 型別加固＝**本輪唯一產品碼改動**：值不是 list → 當空清單＋warning，原本 `list(5 or [])` 會 TypeError 變毒訊息、`list("abc")` 安靜變三個假資料夾；`from_lines` 補 `re.I`；崩潰重送遇 `CloudRouteOff` 照樣 fallback；別人的訊息時 `store.get` 丟例外往外丟）＋93 review 的 minors（policy 形狀一顆、`<INSTANCE_ID>` 佔位、具名 JSON 斷言、aud 訊息）；驗收包 `docs/plan/report/2026-09-03-增量六驗收包-請產品負責人確認.md`（A 段實跑值、B〜E 待產品負責人、D 段 Demo 3 待 push）。**測試 692 → 716（0 skipped；三死埠合指／各單指皆 716；規格 binder 27 零 skip）、端點恆 22、`docs/spec/`／compose／Dockerfile／db／requirements／`test.yml` 零改動**；README／CLAUDE.md／LAUNCH.md 的 543 改 716。全程識別字英文（`test_中文` 名與 conftest 既有中文 fixture 例外）。裁決紀錄在 `.superpowers/sdd/phase0903-1/progress.md`（R0〜R25；R19〜R25 是最終 review 之後的裁決）。
...
# ⚠ 換機之後（92-B：Terminate 92-A 再開 g4dn）實例 ID 會變，**兩處**要一起改：
#    ① 重跑 put-role-policy（policy 裡的 <INSTANCE_ID>）② gh variable set EC2_WORKER_INSTANCE_ID
#    少改一處 CD 不會紅——SendCommand 只是 AccessDenied 或打到舊 ID，安靜半殘。

...
# 改了東西要怎麼生效（--reload 救不了的四種情況）
#   改 .env          → docker compose -f compose.yaml -f compose.dev.yaml restart app
#                      ⚠ 但 DATABASE_URL 與 OLLAMA_BASE_URL 由 compose.yaml 的 environment
#                        覆蓋，在容器裡改 .env 的這兩行怎麼 restart 都不會變（刻意的）
#   改 requirements  → docker compose build app，再 up -d
#   改 certs/        → 同上 restart app（HTTPS 行程握著舊憑證）
